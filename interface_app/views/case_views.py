...
def test_case_model_to_dict(case: TestCase):
    ...
    if case.create_time:
        case_dict['create_time'] = datetime.datetime.strftime(case.create_time,'%Y-%m-%d %H:%M')
    if case.update_time:
        case_dict['update_time'] = datetime.datetime.strftime(case.create_time,'%Y-%m-%d %H:%M')
    case_dict['request_body'] = json.loads(case.request_body,encoding="utf-8")  # loads()将Json字符串解码成python对象
    case_dict['response_assert'] = json.loads(case.response_assert, encoding="utf-8")
    return case_dict
...
class TestCaseView(View):
    ...
    def get(self, request, case_id, *args, **kwargs):
        ...
        if not case:
            return response_failed(code=ErrorCode.TestCase, message='数据不存在')
...
class TestCasesView(View):
    create_schema = Schema({
        'service_id': int,
        'name': And(str, lambda s: 0 < len(s) < 256),
        'method': int,
        'url': str,
        'request_type': int,
        Optional('request_body'): dict,
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not size.isdigit() or not size:  # 先判断是否是数字,是否为空。
            size = 10
        if not page.isdigit() or not page:  # isdigit() 方法检测字符串是否只由数字组成
            page = 10
        size = int(size)
        page = int(page)
        ...
        page_cases = p.page(page)   # 取第page页
        total = p.count  # 总共元素

        for item in page_cases.object_list:  # Page.object_list: 当前页上所有对象的列表
    ...
    def post(self, request, *args, **kwargs):
        ...
        if data.get('request_body'):
            data['request_body'] = json.dumps(data['request_body'])  # dumps()将python对象编码成Json字符串
        if data.get('response_assert'):
            data['response_assert'] = json.dumps(data['response_assert'])

        case = TestCase.objects.create(**data)

        case = TestCase.objects.get(id=case.id)  # 报错request_body是字典，重新获取得到的是字典dict就好了，why？
...
class TestCaseDebugView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not self.debug_schema.is_valid(data):
            return response_failed()
        # print("debug="+str(self.debug_schema.is_valid(data)))
        data = self.debug_schema.validate(data)
        #if data.get('request_body'):
          #  data['request_body'] = json.dumps(data['request_body'])  # dumps()将python对象编码成Json字符串

        response = HttpRequest.send_request(data['url'], data['method'], data.get('request_body', {}), data['request_type'])
